...
if [[ "$operation" == "apply" ]]; then
    ...
    patch="25.09.100"
    ostree_feed_repo="/var/www/pages/feed/rel-25.09/ostree_repo/"
    ostree_extra_repo="/opt/software/rel-${patch}/extra/ostree_repo/"
    if [[ -f "/opt/software/metadata/deploying/WRCP-${patch}-metadata.xml" ]]; then
        metadata_file="/opt/software/metadata/deploying/WRCP-${patch}-metadata.xml"
    else
        metadata_file="/opt/software/metadata/unavailable/WRCP-${patch}-metadata.xml"
    fi
