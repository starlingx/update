...
# TODO(kssurkar): Remove when stx.11 is no longer a supported from-release.
class EtcdUpgradeRollbackHook(BaseHook):
    """Manage etcd stage0 symlink during upgrade/rollback.

    On upgrade:
      - If the stage0 symlink already exists (stx.12+), nothing to do.
      - If not (stx.11), creates it using a known etcd version from
        ETCD_KNOWN_VERSIONS. The version is hardcoded because
        /opt/platform/ (DRBD) is inaccessible on standby controllers
        during deploy-host, so hieradata cannot be read.

    On rollback:
      - Always removes the stage0 symlink.
      - For stx.12: puppet recreates it on boot from hieradata.
      - For stx.11: the symlink is not used by that release.
    """
    ...
    # Only stx.11 needs this; stx.12+ already has the symlink.
    ETCD_KNOWN_VERSIONS = {
        "25.09": "3.4.37",
    }

    def _create_stage0_symlink(self, etcd_version, check_root="/"):
        """Create /var/lib/etcd/stage0 symlink.

        The symlink format is:
        /var/lib/etcd/stage0 -> /usr/local/etcd/<version>/stage0

        The check_root parameter is used to verify the target exists
        ...
        LOG.info("Created etcd stage0 symlink: %s -> %s"
    ...
    def _upgrade(self):
        ...
        For stx.12+: the symlink already exists, nothing to do.
        For stx.11: no versioned etcd infrastructure exists, so the
        symlink must be created using the known version from
        ETCD_KNOWN_VERSIONS.
        """
        ...
        # Symlink absent means from_release lacks versioned etcd
        # infrastructure (i.e., stx.11). Use known version.
        etcd_version = self.ETCD_KNOWN_VERSIONS.get(self._from_release)
        if not etcd_version:
            LOG.warning("No known etcd version for %s and no "
                        "symlink exists, skipping" % self._from_release)
            return

        LOG.info("Creating stage0 symlink with version %s "
                 "for from_release %s"
                 % (etcd_version, self._from_release))
        self._create_stage0_symlink(etcd_version,
                                    check_root=self.TO_RELEASE_OSTREE_DIR)
        LOG.info("EtcdUpgradeRollbackHook upgrade finished.")

    def _rollback(self):
        """Remove etcd stage0 symlink during rollback.

        The symlink is always removed because:
        - stx.12: puppet recreates it on boot from hieradata.
        - stx.11: does not use versioned etcd, symlink not needed.
        """
        LOG.info("Starting EtcdUpgradeRollbackHook rollback.")

        if os.path.islink(self.ETCD_STAGE0_LINK):
            os.unlink(self.ETCD_STAGE0_LINK)
            LOG.info("Removed stage0 symlink for rollback to %s"
                     % self._to_release)
        else:
            LOG.info("stage0 symlink does not exist, nothing to remove")
